...
import {
    ...
    UpsertSummarySchema,
    listSummariesDocs,
    listArticleSummariesDocs,
    getArticleSummaryDocs,
...
) => buildFieldSelection(summaryTable, fields, FORBIDDEN_COLUMNS, {
...
async function upsertSummary(
    ...
    const existing = await db.query.summaries.findFirst({
        where: and(eq(schema.summaries.articleId, article.id), eq(schema.summaries.tone, tone)),
        columns: { id: true }
    })
    ...
    const [saved] = await db.insert(schema.summaries)
        .values({ articleId: article.id, tone, summary: summaryText, embedding })
        .onConflictDoUpdate({
            target: [schema.summaries.articleId, schema.summaries.tone],
            set: { summary: summaryText, embedding, updatedAt: new Date() }
        })
        .returning({
    ...
    return {
        created: !existing,
        summary: { articleExternalId: article.externalId, ...saved } satisfies SummaryRecord
    }
}

summaryCollection.get('/', describeRoute(listSummariesDocs), vValidator('query', SummaryCollectionQuerySchema), async (c) => {
    ...
    return c.json({ summaries: rows, meta: { page, limit, count: rows.length, total } })
})

summaries.get('/', describeRoute(listArticleSummariesDocs), vValidator('param', SummaryArticleParamsSchema), vValidator('query', SummaryFieldsQuerySchema), async (c) => {
...
summaries.get('/:tone', describeRoute(getArticleSummaryDocs), vValidator('param', SummaryToneParamsSchema), vValidator('query', SummaryFieldsQuerySchema), async (c) => {
    ...
    const [summary] = await db.select(selection)
        .from(summaryTable)
        .innerJoin(articleTable, eq(summaryTable.articleId, articleTable.id))
        .where(and(eq(summaryTable.articleId, article.id), eq(summaryTable.tone, tone)))
        .limit(1)
...
summaries.put('/:tone', requireWriteAuth(), describeRoute(upsertArticleSummaryDocs), vValidator('param', SummaryToneParamsSchema), vValidator('json', UpsertSummarySchema), async (c) => {
    const { id: externalId, tone } = c.req.valid('param')
    ...
    try {
        ...
        if (error instanceof SummaryEmbeddingError) return c.json({ error: 'Embedding service failed' }, 502)
